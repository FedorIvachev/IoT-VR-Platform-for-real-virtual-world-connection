...
The platform has already been used for research in the VR-IoT environment.
Students of the Autumn 2020 HCIT course were required to provide research on an interaction method in a specific Smart environment simulated in Virtual reality for their course projects. Each team was given an Oculus Quest 2 VR headset for testing their demos. Students had access to the previous version of the NUIX-Studio platform based on the fourth prototype at the time of the coursework development. As mentioned in chapter 6, this prototype did not have the functionality of convenient creation of devices, simultaneous work, and integration of most IoT devices from the real world. However, the platform provided tools for interaction with virtual IoT devices. Students integrated the platform into their projects written in Unity by adding Widgets such as Sight Sensor, gesture recognition, video streaming, and virtual controls, and tested various hypotheses for interacting with devices in Virtual reality. As a result, the platform helped to achieve important results in the course projects of students. 

Next, selected course projects will be presented. For each of the projects, the research subject is specified first, followed by the experiment and results sections.
...
\begin{enumerate}
    \item Baseline scheme: basic positioning of the device based on the sound feedback;
    \item Team solution: positioning of the device based on the spatial location and sound changes: when the angle between the user's current orientation and the user-device connection direction is smaller, the volume is louder.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item The test group's maximum time is about 110s, which is much slower than the baseline. The test group's time was affected by the two factors of the object distance and corresponding angle.
\end{enumerate}
...
In the second experiment, most users mentioned that the interaction efficiency had been significantly improved using the Team solution. However, there is little difference in terms of learning costs and fatigue levels because processing the sound information for finding objects is not as direct as using visual information. 
...
People often use multiple devices with similar functionality. For example, TVs, computers, and mobile phones all have screens. The traditional method to switch a certain display content from one device to another includes using buttons, remote controls, etc. This team developed novel triggering methods to provide users a more natural, comfortable experience.
...
The second method is based on using the Location Widget. The device activation sensor triggers within a certain distance between the user and the screen.
...
The easiest to use method is sight-based interaction(Figure~\ref{fig:Project10-figure}), while the gesture-based method provides the most naturalness, sense of control and user satisfaction.

The failure rate of gesture recognition is 5\%, primarily because of the instability of the device's hand recognition.
The user's low evaluation of button control is mainly due to the lack of tactile feedback in VR and unstable hand recognition, making the button's triggering fail to be successfully pressed or double-clicked.

The basis for fast device switching is the remote interconnection of multiple devices. Although the current Smart home environments have not fully achieved this goal, the team believes this is an inevitable trend in the future.\footnote{Using the final prototype of NUIX-Studio, researchers can implement remote interconnection between real and virtual devices}
...
This project does not present much novelty, but it demonstrates how the NUIX-Studio platform can be easily integrated into a specific Smart environment.
...
This team designed a new type of interaction method based on smart gloves usage, helping patients interact with devices through gestures and hand movements. Smart gloves can be represented in VR by hand recognition technology.

\subsubsection{Experiment}

The team has added several gestures support into the gesture recognition Widget to control the objects inside a virtual hospital environment (Figure~\ref{fig:Project4-figure}).
...
Overall the team has implemented a unique instrument, which can be added to NUIX-Studio as a Widget for Smart classroom environment.
...
By analyzing the time for the user to wake up the UI and make a choice in the experiment, the following results were obtained:
\begin{enumerate}
    \item Both waking up the UI and making a choice is easy to be learned how to perform by the user;
    \item After the user is fully familiar with the two interaction methods and scenarios, it takes 8.3s and 9.5s on average to perform the selection by the gaze and body interactions, respectively (this includes the 3.5s time required to wake up the UI and confirm the choice). As a result, the body interaction method is relatively fast and convenient;
    \item The users succeeded in a total of 101 UI wake-up operations, and there was no false triggering of UI wake-up. Among the 70 sight selections, there were 5 cases where the user intended to confirm the option but did not succeed, but there was no case where the wrong option was confirmed. 
    \item Most participants agreed that line-of-sight interaction is more natural. The team believes that this may come from the inconvenience caused by the discomfort when wearing VR.
\end{enumerate}

\section{Spring Semester Student Course Projects}

Students of the Spring 2021 HCIT course provided interesting examples of using IoT devices for messaging, listening to music, reading news and friends' news feed, watching videos and creating memos in different environments. Thus, it is necessary to provide students with a more detailed tutorial on using the platform and add new examples of interaction with Widgets. In addition, the author proposes to create a way to distribute the  Widgets between the teams so that students could focus more on the HCI aspects and User study. 